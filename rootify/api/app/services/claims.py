...
async def extract_and_store_youtube_claims(db, artist_id):
    ...
    candidates = []
    artist_name_norm = _normalize_name(artist.name)
    for sec in sections:
        pattern_candidates = extract_influence_candidates(sec.text, sec.section_path)
        string_candidates = await extract_candidates(db, sec.text)

        seen_names: set[str] = set()
        for candidate in pattern_candidates:
            if _normalize_name(candidate.get("influence_artist")) == artist_name_norm:
                continue
            seen_names.add(_normalize_name(candidate.get("influence_artist")))
            candidates.append(candidate)

        for candidate in string_candidates:
            influence_artist = candidate.influence_artist
            influence_artist_norm = _normalize_name(influence_artist)
            if influence_artist_norm == artist_name_norm or influence_artist_norm in seen_names:
                continue
            candidates.append(
                {
                    "influence_artist": influence_artist,
                    "pattern_type": "weak",
                    "section_path": sec.section_path,
